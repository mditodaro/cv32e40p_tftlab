...
VSIM_FLAGS      = -suppress 3015 -suppress 3448 +dumpports+nocollapse
...
RTLSRC_TB		:= $(filter-out $(TB_DIR)/cv32e40p_fp_wrapper.sv,\
				$(wildcard $(TB_DIR)/*.sv))
...
help::
	...
	@printf "\033[31m\tquesta/lsim/compile\033[39m Invokes Siemens QuestaSIM to compile RI5CY RT-level and Gate-level descriptions for logic simulation\n"
	...
	@printf "\033[31m\tquesta/lsim/gate/shell \033[39m Invokes QuestaSIM to perform an Gate-level simulation of the sbst in shell mode.\n"
	@printf "\033[31m\tquesta/lsim/gate/gui \033[39m Invokes QuestaSIM to perform an Gate-level simulation of the sbst in gui mode.\n"
...
.questa-compile: $(CURDIR)/run/questasim
	cd $(RUN_DIR); $(VLIB) work_gate

	cd $(RUN_DIR); $(VLOG) -work work_gate                      \
		$(DEFINE_FUNCTIONAL)                                    \
		${TECHLIB_DIR}/NangateOpenCellLibrary.v

	cd $(RUN_DIR); $(VLOG) -sv -work work_gate -cover t         \
		-timescale "1 ns/ 1 ps"                                 \
		$(VLOG_FLAGS)                                			\
	...
	cd $(RUN_DIR); $(VOPT) -work work_gate  \
		$(VOPT_FLAGS) $(VOPT_SDF) \
